module aer_ccn_act_mod
    ...
    integer :: npop, NSPCS !matrix information
    character(len=32) :: act_double_call = 'no_double_call'
    character(len=32) :: act_ccn_type    = 'yim'
    ...
    namelist /aer_ccn_act_nml/ nooc, sul_concen, low_concen, high_concen, &
        ...
        lowT2, highT2, act_double_call, act_ccn_type
...
contains
    ...
    subroutine aer_ccn_act_wpdf_m(T, p, wm, wp2, offs, totalmass, drop, &
                                ...
                                matrix_num, matrix_dg_dry, matrix_mspcs, matrix_sigma) !XL
        ...
        integer :: tym, ier, allocate_size
        character(len=256) :: ermesg
        integer :: tpop, tspcs
        real, optional,   intent(out)   :: drop_diag
        real, optional,   intent(inout)   :: drop_pop(:), drop_pop_mass(:), drop_pop_diag(:), drop_pop_mass_diag(:)
        real, optional,   intent(in)    :: matrix_num(:), matrix_dg_dry(:), matrix_mspcs(:,:), matrix_sigma(:)
        ...
        call aer_ccn_act_wpdf_m_k (T, p, wm, wp2, offs, totalmass, tym,       &
            ...
            drop_pop, drop_pop_mass, &
            drop_pop_diag, drop_pop_mass_diag, &
            matrix_num, matrix_dg_dry, matrix_mspcs, matrix_sigma)
    ...
    subroutine aer_ccn_act_init ()
        ...
        integer :: ier_k_ini = 0
        character(len=256) :: ermesg_k_ini


        if (module_is_initialized) return
        call query_matrix_info(npop, NSPCS) !npop and nspcs are module variables
        !--------------------------------------------------------------------- 
        !    read namelist.
        !--------------------------------------------------------------------
        ...
        if (mpp_pe() == mpp_root_pe() ) then
                write(*,*) 'aer_ccn_act_ini matrix_info, npop, nspcs = ', npop, nspcs
        endif
        ...
        call aer_ccn_act_k_init (droplets,   &
            ...
            highmass2, lowmass3, highmass3,  &
            lowmass4, highmass4, lowmass5, highmass5, &
            lowT2, highT2, act_ccn_type, act_double_call, ier_k_ini, ermesg_k_ini)
    if (ier_k_ini /= 0) call error_mesg ('aer_ccn_act_init', ermesg_k_ini, FATAL)
