module matrix_gfdl
    ...
    integer:: id_RI_2 = -1, id_LI_2 = -1, id_BI_2 = -1, id_half_KNN_2 = -1, id_num_pop_i = -1
    integer :: id_KBAR0_22 = -1, id_KBAR3_22 = -1 !ACC_ACC KBAR value, KBAR unit m3/s
...
contains
    ...
    subroutine matrix_run(r, pfull, rh, t, dt, pwt, zhalf, rdt_matrix, Time, is,ie,js,je)
        ...
        if (do_matrix) then
            ...
            if (do_coag) then
                ...
                !------------------------------------------------------------------------------
                call matrix_coag(rt,pfull, t, RI, LI, RIM, LIM, is,ie,js,je, time) !LI/LIM positive number
                ...
                if ((matrix_all_pop(n)%nb_tracer_pop > 0) .and. (lowercase(trim(matrix_all_pop(n)%name)) .ne. "ext")) then
                    ...
                    elseif (matrix_all_pop(n)%I_MDUST > 0) then
                        matrix_all_tracer(matrix_all_pop(n)%I_MDUST)%source(is:ie,js:je,:) = &
                            matrix_all_tracer(matrix_all_pop(n)%I_MDUST)%source(is:ie,js:je,:) &
                            + RIM(n,2,:,:,:) - LIM(n,2,:,:,:)
                    elseif (matrix_all_pop(n)%I_MSEAS > 0) then
                        matrix_all_tracer(matrix_all_pop(n)%I_MSEAS)%source(is:ie,js:je,:) = &
                            matrix_all_tracer(matrix_all_pop(n)%I_MSEAS)%source(is:ie,js:je,:) &
                            + RIM(n,3,:,:,:) - LIM(n,3,:,:,:)
                    elseif (matrix_all_pop(n)%I_MOCAR > 0) then
                        matrix_all_tracer(matrix_all_pop(n)%I_MOCAR)%source(is:ie,js:je,:) = &
                            matrix_all_tracer(matrix_all_pop(n)%I_MOCAR)%source(is:ie,js:je,:) &
                            + RIM(n,4,:,:,:) - LIM(n,4,:,:,:)
                    elseif (matrix_all_pop(n)%I_MBCAR > 0) then
                        matrix_all_tracer(matrix_all_pop(n)%I_MBCAR)%source(is:ie,js:je,:) = &
                            matrix_all_tracer(matrix_all_pop(n)%I_MBCAR)%source(is:ie,js:je,:) &
                            + RIM(n,5,:,:,:) - LIM(n,5,:,:,:)
                    endif
                endif
            ...
            if (do_AQSO4) then
                ...
                if ((matrix_all_pop(n)%nb_tracer_pop > 0) .and. (lowercase(trim(matrix_all_pop(n)%name)) .ne. "ext") &
                    ...
                    used = send_data(matrix_all_pop(n)%id_pop_aqso4_partition, &
                        AQSO4(is:ie,js:je,:)*matrix_all_tracer(matrix_all_pop(n)%I_N)%value_in_matrix(is:ie,js:je,:)/total_aero_num, time, &
                        is_in=is,js_in=js, ks_in=1)
                endif
                enddo
            ...
            if (matrix_all_tracer(n)%is_active) then
                matrix_all_tracer(n)%value_in_matrix(is:ie,js:je,:) = & 
                    matrix_all_tracer(n)%value_in_matrix(is:ie,js:je,:) + matrix_all_tracer(n)%source(is:ie,js:je,:) * dt

                if (matrix_all_tracer(n)%id_value_bf_max > 0) then
                    used = send_data(matrix_all_tracer(n)%id_value_bf_max, matrix_all_tracer(n)%value_in_matrix(is:ie, js:je,:), time, &
                        is_in=is,js_in=js,ks_in=1)
                endif

                matrix_all_tracer(n)%value_in_matrix(is:ie,js:je,:) = &
                    max(matrix_all_tracer(n)%value_in_matrix(is:ie,js:je,:), 0.0) !value can't < 0


                if (matrix_all_tracer(n)%id_value_af_max > 0) then
                    used = send_data(matrix_all_tracer(n)%id_value_af_max, matrix_all_tracer(n)%value_in_matrix(is:ie, js:je,:), time, &
                        is_in=is,js_in=js,ks_in=1)
                endif
            endif
            enddo
            ...
            !-----------------------------------------------------------------------------------------------------------------
            do n=1, ntrace
            if (matrix_all_tracer(n)%id_tracer_tsource > 0) then
                if (matrix_all_tracer(n)%type .eq. 'mass') then
                    used = send_data(matrix_all_tracer(n)%id_tracer_tsource, matrix_all_tracer(n)%source(is:ie, js:je,:), time, &
                        is_in=is,js_in=js,ks_in=1)
                elseif(matrix_all_tracer(n)%type .eq. 'number') then
                    used = send_data(matrix_all_tracer(n)%id_tracer_tsource, matrix_all_tracer(n)%source(is:ie, js:je,:), time, &
                        is_in=is,js_in=js,ks_in=1)
            ...
            if (matrix_all_tracer(n)%id_tendency_gfdl > 0) then
                if (matrix_all_tracer(n)%type .eq. 'mass') then
                    used = send_data(matrix_all_tracer(n)%id_tendency_gfdl, rdt_matrix(:,:,:,n), time, &
                        is_in=is,js_in=js,ks_in=1)
                elseif(matrix_all_tracer(n)%type .eq. 'number') then
                    used = send_data(matrix_all_tracer(n)%id_tendency_gfdl, rdt_matrix(:,:,:,n), time, &
                        is_in=is,js_in=js,ks_in=1)
    ...
    subroutine matrix_init(r, axes, time)
        ...
        if (matrix_all_pop(n)%nb_tracer_pop > 0 ) then
            ...
            case ('dd1')
                matrix_all_pop(n)%rh_deliquescence = 0.8
                matrix_all_pop(n)%rh_crystallization = 0.2
                matrix_all_pop(n)%dens_dry = 2500
            case ('dd2')
                matrix_all_pop(n)%rh_deliquescence = 0.8
                matrix_all_pop(n)%rh_crystallization = 0.2
                matrix_all_pop(n)%dens_dry = 2650
            case ('ssa')
                matrix_all_pop(n)%rh_deliquescence = 0.8
                matrix_all_pop(n)%rh_crystallization = 0.2
        ...
        id_RI_2 = register_diag_field ( module_name, 'RI_ACC', axes(1:3),Time, 'RI_ACC', '#/m3/s', &
            missing_value=-999.  )
        id_LI_2 = register_diag_field ( module_name, 'LI_ACC', axes(1:3),Time, 'LI_ACC', '#/m3/s', &
            missing_value=-999.  )
        id_BI_2 = register_diag_field ( module_name, 'BI_ACC', axes(1:3),Time, 'BI_ACC', '#/s', &
            missing_value=-999.  )
        id_half_KNN_2 = register_diag_field ( module_name, 'KNN_ACC', axes(1:3),Time, 'KNN_ACC', '#/m3/s', &
            missing_value=-999.  )
        id_num_pop_i = register_diag_field ( module_name, 'NUM_POP_ACC', axes(1:3),Time, 'NUM_POP_ACC', '#/m3', &
            missing_value=-999.  )
        id_KBAR0_22 = register_diag_field ( module_name, 'KBAR0_ACC', axes(1:3),Time, 'KBAR0_ACC', 'm3/s', &
            missing_value=-999.  )
        id_KBAR3_22 = register_diag_field ( module_name, 'KBAR3_ACC', axes(1:3),Time, 'KBAR3_ACC', 'm3/s', &
            missing_value=-999.  )
    ...
    subroutine set_matrix_source_generic(source_type,source_processed, zhalf, time, is,js)
        ...
        if (do_matrix) then
            ...
            if (source_type .eq. matrix_source_type%P_H2SO4) then   
                ...
                enddo
                used = send_data (id_h2so4_source, h2so4_source_2D, time, &
                    is_in=is,js_in=js)
            ...
            elseif (source_type .eq. matrix_source_type%P_AQSO4) then
                ...
                used = send_data (id_AQSO4_rate, AQSO4(is:ie,js:je,:), time, is_in=is,js_in=js, ks_in=1)
    ...
    !----------------------------------------------------------------------------------------------------------------
    subroutine matrix_coag(r,pfull, t, RI, LI, RIM, LIM, is,ie,js,je, time)
        implicit none
        logical :: used
        type(time_type),  intent(in) :: Time
        integer, intent(in) :: is,ie,js,je
        ...
        used = send_data(id_KBAR0_22, KBAR0_IJ(2,2,:,:,:), time, is_in=is,js_in=js, ks_in=1)
        used = send_data(id_KBAR3_22, KBAR3_IJ(2,2,:,:,:), time, is_in=is,js_in=js, ks_in=1)
        ...
        used = send_data(id_RI_2, RI(2,:,:,:), time, is_in=is,js_in=js, ks_in=1)
        ...
        used = send_data(id_BI_2, BI(2,:,:,:), time, is_in=is,js_in=js, ks_in=1)
        ...
        used = send_data(id_num_pop_i, num_pop_i, time, is_in=is,js_in=js, ks_in=1)
    ...
    subroutine matrix_dry_diameter(r,is,ie,js,je)
        ...
        if ((matrix_all_pop(n)%nb_tracer_pop > 0) .and. (lowercase(trim(matrix_all_pop(n)%name)) .ne. "ext"))  then
            ...
            enddo
            if (flag > 0) then
                m_dry_all = max(m_dry_all, 1.0e-32)
                vol_dry_all = max(vol_dry_all, 1.0e-44)
                ...
                pop_num = max(matrix_all_tracer(matrix_all_pop(n)%I_N)%value_in_matrix(is:ie,js:je,:), 1.e-32) !#/m3
                ...
                matrix_all_pop(n)%Dg_dry(i+is-1,j+js-1,k) = &
                    max(1.0e-10, (m_dry_all(i,j,k)/pop_num(i,j,k)/dens_dry(i,j,k)*1E-9/PI6)**(1.0/3)/exp_fac ) ! Dg_dry unit: m
                ...
                !    dens_dry(i,j,k) = 1000
                !    matrix_all_pop(n)%dens_dry(i+is-1,j+js-1,k) = dens_dry(i,j,k)
                !    matrix_all_pop(n)%kappa_pop(i+is-1,j+js-1,k)=1.0e-32
                !    matrix_all_pop(n)%Dg_dry(i+is-1,j+js-1,k) = 1.0e-10
                !endif
                enddo
                enddo
                enddo
    ...
    subroutine matrix_wet_diameter(r, rh, t, is,ie,js,je) !calculate and assgin values for pop%Dg_wet
        ...
        if (flag > 0) then
            ...
            ddry_in_3d = max(ddry_in_3d, 1.0e-10)
            ...
            !                        hygro_in, s_in
            !endif
            matrix_all_pop(n)%Dg_wet(i+is-1,j+js-1,k) = min(5.0e-5, matrix_all_pop(n)%Dg_wet(i+is-1,j+js-1,k))
            gf3 = gf**3
    ...
    subroutine matrix_sedimentation(T, P, zhalf, dt, is, ie, js, je)
        ...
        if ((matrix_all_pop(n)%nb_tracer_pop > 0) .and. (lowercase(trim(matrix_all_pop(n)%name)) .ne. "ext") &
            ...
            rho_wet = matrix_all_pop(n)%dens_wet(is:ie,js:je,:)
            ...
            do nt = 1, matrix_all_pop(n)%nb_tracer_pop
            ntt = matrix_all_pop(n)%tracer_index(nt)
            conc = matrix_all_tracer(ntt)%value_in_matrix(is:ie, js:je, :) !ug/m3 or #/m3
            !flux at TOA
    ...
    subroutine set_config_pop(matrix_configuration)
        ...
        if (matrix_configuration .eq. "debug") then
            ...
            if (.not. allocated(I_POP)) &
                allocate(I_POP(pop_active))
            I_POP(:)=(/I_ACC/)
        elseif (matrix_configuration .eq. "akk_acc") then
            I_AKK = 1
            I_ACC = 2
            I_EXT = 13
            pop_active = 2
