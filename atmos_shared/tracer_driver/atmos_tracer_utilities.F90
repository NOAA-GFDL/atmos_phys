module atmos_tracer_utilities_mod
    ...
    use matrix_gfdl, only : matrix_query_dynamic_wetdep_param_2D_mass
    implicit none
    private
    !-----------------------------------------------------------------------
    !----- interfaces -------
    ...
    public  wet_deposition,    &
        ...
        sedimentation_flux
    !query_caerosol_wetdep_param !XL
    ...
    !for matrix to use
    real :: dust5_frac_in_cloud=0., dust5_frac_in_cloud_snow=0., dust5_frac_in_cloud_snow_homogeneous=0.
    real :: dust5_alpha_r=0., dust5_alpha_s=0., dust5_frac_in_cloud_uw=0.
    real :: ssalt5_frac_in_cloud=0., ssalt5_frac_in_cloud_snow=0., ssalt5_frac_in_cloud_snow_homogeneous=0.
    real :: ssalt5_alpha_r=0., ssalt5_alpha_s=0., ssalt5_frac_in_cloud_uw=0.
    ...
    type wetdep_type
        ...
        real  :: Henry_variable
        real  :: frac_in_cloud
        real  :: frac_in_cloud_uw !XL
        real  :: frac_in_cloud_snow
        real  :: frac_in_cloud_snow_homogeneous
        real  :: alpha_r
        real  :: alpha_s
        logical :: Lwetdep, Lgas, Laerosol, Lice, so2_so4_evap, is_so2, Ldep_dynamic !XL
...
contains
    ...
    subroutine atmos_tracer_utilities_init(lonb, latb, mass_axes, Time)
        ...
        integer :: dust_flag = 1 !XL
        integer :: ssalt_flag = 2 !XL
        logical  :: flag
        ...
        call get_wetdep_param(Wetdep(n)%text_in_scheme,  &
            ...
            Wetdep(n)%Ldep_dynamic, & !XL
            frac_in_cloud_uw = Wetdep(n)%frac_in_cloud_uw, &
        ...
        if ( lowercase(trim(tracer_names(n))) .eq. 'dust5') then
            dust5_frac_in_cloud = Wetdep(n)%frac_in_cloud
            dust5_frac_in_cloud_snow = Wetdep(n)%frac_in_cloud_snow
            dust5_frac_in_cloud_snow_homogeneous = Wetdep(n)%frac_in_cloud_snow_homogeneous
            dust5_alpha_r = Wetdep(n)%alpha_r
            dust5_alpha_s = Wetdep(n)%alpha_s
            dust5_frac_in_cloud_uw = Wetdep(n)%frac_in_cloud_uw
        elseif ( lowercase(trim(tracer_names(n))) .eq. 'ssalt5') then
            ssalt5_frac_in_cloud = Wetdep(n)%frac_in_cloud
            ssalt5_frac_in_cloud_snow = Wetdep(n)%frac_in_cloud_snow
            ssalt5_frac_in_cloud_snow_homogeneous = Wetdep(n)%frac_in_cloud_snow_homogeneous
            ssalt5_alpha_r = Wetdep(n)%alpha_r
            ssalt5_alpha_s = Wetdep(n)%alpha_s
            ssalt5_frac_in_cloud_uw = Wetdep(n)%frac_in_cloud_uw
        endif
    ...
    !!XL: for matrix use
    !subroutine query_caerosol_wetdep_param(flag, wet_params) !query seasalt and dust statis wet-dep paramters
    !    integer, intent(in) :: flag !query aerosol type flag, flag=1: dust5; flag=2: ssalt5
    !    real, intent(out) :: wet_params(6) !frac_in_cloud, frac_in_cloud_snow,frac_in_cloud_snow_homogeneous, alpha_r, alpha_s 
    !    if (.not. module_is_initialized) then
    !        call error_mesg('wet_deposition', 'module not initialized when query parameters', FATAL) 
    !    endif
    !    if (flag == 1) then
    !        wet_params(1) = dust5_frac_in_cloud
    !        wet_params(2) = dust5_frac_in_cloud_snow
    !        wet_params(3) = dust5_frac_in_cloud_snow_homogeneous
    !        wet_params(4) = dust5_alpha_r
    !        wet_params(5) = dust5_alpha_s
    !        wet_params(6) = dust5_frac_in_cloud_uw
    !    elseif (flag == 2) then
    !        wet_params(1) = ssalt5_frac_in_cloud
    !        wet_params(2) = ssalt5_frac_in_cloud_snow
    !        wet_params(3) = ssalt5_frac_in_cloud_snow_homogeneous
    !        wet_params(4) = ssalt5_alpha_r
    !        wet_params(5) = ssalt5_alpha_s
    !        wet_params(6) = ssalt5_frac_in_cloud_uw
    !    else
    !        call error_mesg('wet_deposition', 'query_caerosol_wetdep_param: flag not defined', FATAL)
    !    endif

    !end subroutine query_caerosol_wetdep_param
    ...
    subroutine dry_deposition_init( n, &
        ...
        if( query_method('dry_deposition',MODEL_ATMOS,n,name,control) )then
            !write(*,*) 'dry_deposition', n, name, control
        ...
        case ('williams_wind_driven')
            ...
            subroutine wet_deposition( n, T, pfull, phalf, zfull, zhalf, &
                ...
                real, dimension(size(T,1),size(T,2)) :: &
                    ...
                    frac_in_cloud_2D, frac_in_cloud_snow_homogeneous_2D, & ! frac_in_cloud_snow_2D, frac_in_cloud_snow_homogeneous_2D, alphar_2D, alphas_2D, & !XL
                    wdep_in, wdep_bc, fluxr,fluxs, tracer_flux, ones_array_2D
                real, dimension(size(T,1),size(T,2),size(pfull,3)) :: &
                ...
                real , parameter :: &
                    ...
                    frac_int_dynamic = 0.5 !XL

                real :: alpha_r, alpha_s

                logical :: &
                    used, &
                    Lwetdep, Lgas, Laerosol, Lice, Ldep_dynamic !XL
                character(len=500) :: &
                ...
                Lice    = Wetdep(n)%Lice
                Ldep_dynamic = Wetdep(n)%Ldep_dynamic !XL
                ...
                if(lowercase(scheme)=='wdep_gas' .or. lowercase(scheme)=='wdep_aerosol') then
                    ...
                    elseif( Laerosol ) then
                        frac_int = frac_int_aerosol
                    elseif( Ldep_dynamic ) then
                        frac_int = frac_int_dynamic !XL
                    else
                        frac_int = 1.
                    end if

                    if( Lgas .or. Laerosol .or. Ldep_dynamic) then !XL
                        ! units = VMR
                        !
                        ...
                        !
                        if( (Lgas .and. Henry_constant > 0) .or. Laerosol .or. Ldep_dynamic ) then
                            in_temp(:,:,:) = 0.
                            bc_temp(:,:,:) = 0.
                            do k=1,kd
                            ...
                            else if (Laerosol .or. Ldep_dynamic) then
                                if (Ldep_dynamic) then
                                    call matrix_query_dynamic_wetdep_param_2D_mass( n, pfull, is, js, k,frac_in_cloud_2D)
                                    frac_in_cloud_2D = frac_in_cloud_2D * scale_aerosol_wetdep
                                    scav_factor(:,:) = frac_in_cloud_2D
                                    if (frac_in_cloud_snow_homogeneous == -1) then!frac_in_cloud_2D equals frac_in_cloud_snow_homogeneous
                                        frac_in_cloud_snow_homogeneous_2D = frac_in_cloud_2D
                                    elseif (frac_in_cloud_snow_homogeneous > -1) then !frac_in_cloud_snow_homogeneous > -1 means it is
                                        frac_in_cloud_snow_homogeneous_2D = frac_in_cloud_snow_homogeneous*ones_array_2D
                                    endif
                                    !if (mpp_root_pe() == mpp_pe()) then
                                    !    write(*,'(a)') 'i   j   n   k   frac_in_cloud_2D   scav_factor   scav_factor_s   frac_in_cloud_snow_hom_2D   f_snow_berg   frac_in_cloud_snow   T_homogeneous   T(i,j,k)'
                                    !    write(*,'(a)') repeat('-', 120)
                                    !endif
                                    do i=1,id
                                    do j=1, jd
                                        if (frac_in_cloud_2D(i,j) == frac_in_cloud_snow) then
                                                scav_factor_s(i,j) = scav_factor(i,j)
                                        else
                                                scav_factor_s(i,j) = f_snow_berg(i,j,k)*frac_in_cloud_snow + &
                                                        (1.-f_snow_berg(i,j,k))*frac_in_cloud_2D(i,j)
                                                if (frac_in_cloud_2D(i,j) .ne. frac_in_cloud_snow_homogeneous_2D(i,j)) then
                                                        if (T(i,j,k) .lt. T_homogeneous) then
                                                                scav_factor_s(i,j) = f_snow_berg(i,j,k)*frac_in_cloud_snow + &
                                                                         (1.-f_snow_berg(i,j,k))*frac_in_cloud_snow_homogeneous_2D(i,j)
                                                        endif
                                                endif
                                        endif
                                     !           if (mpp_root_pe() == mpp_pe()) then
                                     !                   write(*,'(2i4,2i4,1x,6f12.5,2f12.2)') i+is-1, j+js-1, n, k, &
                                     !                   frac_in_cloud_2D(i,j), scav_factor(i,j), scav_factor_s(i,j), &
                                     !                   frac_in_cloud_snow_homogeneous_2D(i,j), f_snow_berg(i,j,k), &
                                     !                   frac_in_cloud_snow, T_homogeneous, T(i,j,k)
                                     !           endif
                                    enddo
                                    enddo
                                endif
                            ...
                            if( lowercase(scheme)=='henry_below' .or. lowercase(scheme)=='henry_below_noice') then
                            ...
                            else if ( lowercase(scheme) == 'aerosol_dynamic' ) then
                                do k=1,kd
                                ...
                                !bc_temp(:,:,k) = 0.75 * &
                                !    (fluxr(:,:)*alphar_2D/R_r/DENS_H2O + &
                                !    fluxs(:,:)*alphas_2D/R_s/DENS_SNOW)
                                enddo
            ...
            subroutine get_wetdep_param(text_in_scheme,text_in_param,scheme,&
                ...
                logical, intent(out)            :: Lwetdep, Lgas, Laerosol, Lice, Ldep_dynamic !XL
                ...
                if( trim(lowercase(text_in_scheme)) == 'fraction' ) then
                    ...
                    Lgas = .true.
                else if ( trim(lowercase(text_in_scheme)) == 'matrix_dep_dynamic') then
                    scheme = 'aerosol_dynamic'
                    Ldep_dynamic = .true. !XL
                    flag=parse(text_in_param,'frac_incloud_snow',frac_in_cloud_snow)
                    if (flag == 0) then
                        call error_mesg('atmos_tracer_utilities_init', 'frac_incloud_snow is not defined for dynamic', FATAL)
                    endif
                    if (present(frac_in_cloud_snow_homogeneous)) then
                        flag=parse(text_in_param,'frac_incloud_snowh',frac_in_cloud_snow_homogeneous)
                        if (flag == 0) then
                            frac_in_cloud_snow_homogeneous = -1 !if frac_incloud_snowh is not defined in field table, it should
                            !equals to frac_in_cloud, hence becomes dynamic 
                            !XL: need to revisit after figureing out
                            !diffrence between frac_in_cloud_snow and frac_in_cloud_snow_homogeneous
                        endif
                    endif
