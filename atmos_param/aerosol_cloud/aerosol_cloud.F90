MODULE aerosol_cloud_mod
    ...
    use matrix_gfdl,                only: query_matrix_info !XL
    implicit none
    private
...
CONTAINS
    ...
    subroutine determine_activated_aerosol (   &
            ...
            diag_4d, diag_id, diag_pt, Aerosol) !XL, pass aerosol here
        ...
        TYPE(aerosol_type),         INTENT (in)   :: Aerosol !XL
            ...
            if (var_limit_opt == 1) then   ! cjg
                ...
                (total_activation) .or.  &
                    ...
                    call aer_ccn_act_wpdf_m   &
                        ...
                        Particles%drop1_diag(i,j,k),  &
                        Particles%drop1_pop(i,j,k,:), Particles%drop1_pop_mass(i,j,k,:), &
                        Particles%drop1_pop_diag(i,j,k,:), Particles%drop1_pop_mass_diag(i,j,k,:), &
                        Aerosol%matrix_N(i,j,k,:),Aerosol%matrix_Dg_dry(i,j,k,:), &
                        Aerosol%matrix_MSPCS(i,j,k,:,:), Aerosol%matrix_sigma(:))
                else
                    Particles%drop1(i,j,k) = 0.
                    !XL
                    Particles%drop1_diag(i,j,k) = 0.
                    Particles%drop1_pop(i,j,k,:) = 0.                   
                    Particles%drop1_pop_diag(i,j,k, :) = 0
                    Particles%drop1_pop_mass(i,j,k,:) = 0
                    Particles%drop1_pop_mass_diag(i,j,k,:) = 0.
                endif
                end do
                end do
                end do
