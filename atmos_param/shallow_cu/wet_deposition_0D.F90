module wet_dep_0D_mod

    use matrix_gfdl, only : grid_wetdep_fic_uw_mass
    implicit none
    private

    public wet_deposition_0D
    !VERSION NUMBER:
    !  $Name$
    !  $Id$
contains
    ...
    subroutine wet_deposition_0D( Henry_constant, Henry_variable, &
            ...
            cloud, rain, snow, &
            tracer, Lgas, Laerosol, Lice, &
            Ldep_dynamic, & !XL:dynamic dep
            ntrace_orig, & !recorded original tracer index
            delta_tracer, mass_grid_spec ) !XL
        implicit none
        !      
        !<OVERVIEW>
        ...
        logical,          intent(in)                     :: Lgas, Laerosol, Lice, Ldep_dynamic !XL
        real,             intent(out)                    :: delta_tracer
        integer,          intent(in)                     :: ntrace_orig !XL: original order for traer index
        real,             intent(in), optional           :: mass_grid_spec(5) !kappa_grid will always co-exist with lwet_dynamic
        !-----------------------------------------------------------------------
        !     ... local variables
        !-----------------------------------------------------------------------
        real :: &
        ...
        if( Lgas .or. Laerosol .or. Ldep_dynamic) then
            !++lwh
            ! units = VMR
            !
            ...
            if( (Lgas .and. Henry_constant > 0.) .or. Laerosol .or. Ldep_dynamic) then
                ...
                else if (Ldep_dynamic) then
                    call grid_wetdep_fic_uw_mass(ntrace_orig, mass_grid_spec, scav_factor) !matrix
                    !kappa_grid_n = 0.3
                    !call grid_wetdep_fic_uw(ntrace_orig, kappa_grid_n, scav_factor) !matrix
                    !!test options for scav_factor
                    !scav_factor = 0.05
                    !!test options 2
                    !scav_factor = kappa_grid
                    !test options 3
                    !scav_factor = 0.001*ntrace_orig
                end if
